...
// src/utils/getLowdefyProjectRoots.ts
var vscode = __toESM(require("vscode"));
var import_node_fetch = __toESM(require_lib3());
async function getLowdefyProjectRoots(context) {
  const config = vscode.workspace.getConfiguration("lowdefyDeTools");
  const port = config.get("localServerPort") || 3e3;
...
// src/views/AppTreeViewProvider.ts
var vscode4 = __toESM(require("vscode"));

// src/views/treeItems/ArrayTreeItem.ts
var vscode2 = __toESM(require("vscode"));
var ArrayTreeItem = class extends vscode2.TreeItem {
...
// src/views/treeItems/TextTreeItem.ts
var vscode3 = __toESM(require("vscode"));
var TextTreeItem = class extends vscode3.TreeItem {
  constructor(label) {
...
var AppTreeViewProvider = class {
  ...
  async getRootChildren() {
    const activeAppRoot = this.context.workspaceState.get("activeAppRoot");
    if (!activeAppRoot) {
      return [];
    }
    const items = [new TextTreeItem("name: Test App"), new TextTreeItem("version: 4.0.2")];
    if (await this.pathExists(
      vscode4.Uri.joinPath(activeAppRoot, ".lowdefy", "dev", "build", "connections")
    )) {
      items.push(new ArrayTreeItem("connections:", vscode4.TreeItemCollapsibleState.Collapsed));
    }
    if (await this.pathExists(vscode4.Uri.joinPath(activeAppRoot, ".lowdefy", "dev", "build", "pages"))) {
      items.push(new ArrayTreeItem("pages:", vscode4.TreeItemCollapsibleState.Collapsed));
    }
...
// src/language/RefLinkProvider.ts
var vscode5 = __toESM(require("vscode"));
var RefLinkProvider = class {
  constructor(context) {
    this.context = context;
  }
  async provideDocumentLinks(document, token) {
    let links = [];
    const activeAppRoot = this.context.workspaceState.get("activeAppRoot");
    const activeAppRootUri = vscode5.Uri.file(activeAppRoot || "");
    ...
    while ((match = regexRef.exec(text)) !== null) {
      ...
      try {
        const uri = vscode5.Uri.joinPath(activeAppRootUri, filePath);
        links.push(new vscode5.DocumentLink(range, uri));
      } catch (err) {
        console.error(
...
async function activate(context) {
  ...
  const disposable = vscode6.languages.registerDocumentLinkProvider(selector, refProvider);
  context.subscriptions.push(disposable);
  vscode6.window.registerTreeDataProvider("lowdefyAppTree", new AppTreeViewProvider(context));
}
function deactivate() {
}
